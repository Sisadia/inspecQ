...
import { Users, Mail, FileText, BarChart3, Calendar, TrendingUp, MessageSquare } from 'lucide-react';
    ...
    <AdminLayout>
      <div className="space-y-8">
        <div>
          ...
          <p className="text-gray-600">Welcome back! Here's what's happening with your admin panel.</p>
        </div>
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Link
              ...
              className="bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 rounded-lg p-4 text-center transition-colors duration-200"
            >
              <Users className="h-8 w-8 text-emerald-600 mx-auto mb-2" />
              <p className="font-medium text-emerald-900">Add New Job</p>
            </Link>
            <Link
              to="/admin/forms"
            ...
            {recentActivity.map((activity, index) => (
              <div key={index} className="flex items-start space-x-4 p-4 bg-gray-50 rounded-lg">
                <div className={`w-2 h-2 rounded-full mt-2 ${
                  activity.type === 'application' ? 'bg-emerald-500' :
                  activity.type === 'newsletter' ? 'bg-blue-500' :
                  activity.type === 'form' ? 'bg-green-500' :
                  'bg-purple-500'
                }`}></div>
